...
class CustomSmartConnect(SmartConnect):
    ...
    def _make_request3(self, *args, **kwargs):
        # Add timeout to kwargs for requests
        kwargs['timeout'] = self.timeout
        return super()._make_request(*args, **kwargs)
    def get_position_with_retry3(api_instance, retries=50, delay=60):
...
def connect_to_smart_api3(api_key, username, password, totp_token):
    retries = 0
    max_retries=60
    retry_delay=5
...
def fetch_candle_data3(api_instance, token, interval, Exchange, gui_app, stop_event,days=9):
    """
...
def fetch_price3(api_instance, token, interval, Exchange, gui_app,days=9):
    """
    Fetch live candle data continuously at a specified interval.
    ="FIVE_MINUTE"
    """
