...
import time
import pandas as pd
from Tabs.Tab2.api_handler2 import fetch_candle_data2
from api_handler import fetch_candle_data


def calculate_macd2(data, short_window, long_window, gui_app, signal_window=9):
    """
...
def process_live_data2(api_instance, token, interval, stop_event, Exchange, gui_app):
    """
    ...
    """
    try:
        data_generator = fetch_candle_data2(api_instance, token, interval, Exchange, gui_app, stop_event)
...
def MACD2(short_window, long_window, gui_app):
    """
        ...
        while True:
            if not price_queue.empty():
                ...
                if not candle_data.empty and not candle_data.isna().all().all():
                    ...
                    data = calculate_macd2(live_candle_data, short_window, long_window, gui_app)
