...
import time
import pandas as pd
from Tabs.Tab4.api_handler4 import fetch_candle_data4



def calculate_macd4(data, short_window, long_window, gui_app, signal_window=9):
    """
...
def process_live_data4(api_instance, token, interval, stop_event, Exchange, gui_app):
    """
    ...
    """
    try:
        data_generator = fetch_candle_data4(api_instance, token, interval, Exchange, gui_app, stop_event)
...
def MACD4(short_window, long_window, gui_app):
    """
        ...
        while True:
            if not price_queue.empty():
                ...
                if not candle_data.empty and not candle_data.isna().all().all():
                    ...
                    data = calculate_macd4(live_candle_data, short_window, long_window, gui_app)
