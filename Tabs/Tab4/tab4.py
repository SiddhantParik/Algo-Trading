...
from Tabs.Tab4.MACD4 import MACD4
from Tabs.Tab4.api_handler4 import fetch_price4
...
class MacdStrategy4:
    ...
    def execute_strategy4(self, api_instance, token, interval, stop_event, call_position, call_token,
        ...
        macd_gen = MACD4(short_window, long_window, gui_app)
        macd_thread = Thread(target=self.macd_calculator_thread, args=(macd_gen, stop_event))
        macd_thread.start()

        price_gen = fetch_price4(api_instance, token, interval, Exchange, gui_app)
        ...
        try:
            while not stop_event.is_set():
                ...
                if not call_exists and not put_exists:
                    ...
                    if macd_value > signal_value and flag == "SELL" and last_action != "Buy PUT":
                        self.place_order(api_instance, put_position, put_token, transaction, quantity, Exchange, gui_app)
                        last_action = "Buy PUT"
                    elif macd_value < signal_value and flag == "BUY" and last_action != "Buy Call":
