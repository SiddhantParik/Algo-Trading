...
import time
import pandas as pd
from Tabs.Tab1.api_handler1 import fetch_candle_data1
from api_handler import fetch_candle_data


def calculate_macd1(data, short_window, long_window, gui_app, signal_window=9):
    """
...
def process_live_data1(api_instance, token, interval, stop_event, Exchange, gui_app):
    """
    ...
    """
    try:
        data_generator = fetch_candle_data1(api_instance, token, interval, Exchange, gui_app, stop_event)
...
def MACD1(short_window, long_window, gui_app):
    """
        ...
        while True:
            if not price_queue.empty():
                ...
                if not candle_data.empty and not candle_data.isna().all().all():
                    ...
                    data = calculate_macd1(live_candle_data, short_window, long_window, gui_app)
